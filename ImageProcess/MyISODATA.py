...
class ClusterPair:
    """Cluster Pair"""
    def __init__(self, initClusterAIndex, initClusterBIndex, initDistance):
...
class ISODATA:
    ...
    def doISODATAGray(self, image, K, TN, TS, TC, L, I):
        ...
        while True:
            ...
            print("Classifying...")
            for row in range(imgX):
                for col in range(imgY):
            ...
            print("Recalculating the centers...")
            for cluster in clusterList:
                sum = 0.0
            ...
            print("Preparing for Merging and Spliting...")
            aveDisctanceList = []
            sumDistanceAll = 0
            for cluster in clusterList:
    ...
    def distanceBetween(self, colorA, colorB):
        ...
        return (2 + aveR / 256) * (dR ** 2) + 4 * (dG ** 2) + (2 + (255 - aveR) / 256) * (dB ** 2)

    def doISODATARGB(self, image, K, TN, TS, TC, L, I):
        imgArray = numpy.array(image)
        ...
        for i in range(K):
            ...
            duplicated = False
            for cluster in clusterList:
                if cluster.center[0] == imgArray[randomX, randomY, 0] and cluster.center[1] == imgArray[randomX, randomY, 1] and cluster.center[2] == imgArray[randomX, randomY, 2]:
                    duplicated = True
                    break
            if not duplicated:
                clusterList.append(Cluster(numpy.array([imgArray[randomX, randomY, 0], imgArray[randomX, randomY, 1], imgArray[randomX, randomY, 2]], dtype = numpy.uint8)))
        ...
        while True:
            ...
            for row in range(imgX):
                for col in range(imgY):
                    targetClusterIndex = 0
                    targetClusterDistance = self.distanceBetween(imgArray[row, col], clusterList[0].center)
                    # Classify
                    for i in range(1, len(clusterList)):
                        currentDistance = self.distanceBetween(imgArray[row, col], clusterList[i].center)
                        if currentDistance < targetClusterDistance:
                            targetClusterDistance = currentDistance
                            targetClusterIndex = i
                    clusterList[targetClusterIndex].pixelList.append(Pixel(row, col, imgArray[row, col]))
            ...
            for cluster in clusterList:
                ...
                if aveR != cluster.center[0] and aveG != cluster.center[1] and aveB != cluster.center[2]:
                    didAnythingInLastIteration = True
            ...
            for cluster in clusterList:
                ...
                for pixel in cluster.pixelList:
                    currentSumDistance += self.distanceBetween(pixel.color, cluster.center)
                aveDisctanceList.append(float(currentSumDistance) / len(cluster.pixelList))
                sumDistanceAll += currentSumDistance
            aveDistanceAll = float(sumDistanceAll) / (imgX * imgY)
            print("Finished.")
            ...
            if (len(clusterList) <= K / 2) or not (iterationCount % 2 == 0 or len(clusterList) >= K * 2):
                ...
                for i in range(len(clusterList) - 1, -1, -1):
                    ...
                    if (maxSD > TS) and ((aveDisctanceList[i] > aveDistanceAll and len(clusterList[i].pixelList) > 2 * (TN + 1)) or (len(clusterList) < K / 2)):
                        ...
                        clusterList.append(Cluster(numpy.array([clusterList[i].center[0], clusterList[i].center[1], clusterList[i].center[2]], dtype = numpy.uint8)))
                ...
                for i in range(0, len(clusterList)):
                    for j in range(0, i):
                        currentDistance = self.distanceBetween(clusterList[i].center, clusterList[j].center)
                        if currentDistance < TC:
                            clusterPairList.append(ClusterPair(i, j, currentDistance))
        ...
        result_path = "C:\\2016302590109\\isodata_img.bmp"
        result_name = "isodata_img"
        result = Image.fromarray(newImgArray, mode="RGB")
        result.save(result_path, "BMP")
        return result_path
